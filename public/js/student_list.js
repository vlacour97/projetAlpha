/**
 * Created by valentinlacour on 27/11/16.
 */


$(function(){

    var url = 'private/controller/students/student_list.php';
    var choice = "";
    var part = 1;
    var res;
    var currentLineId;
    ...
    function initDataTables(){
        ...
        $('#student-table').find('thead th').each(function(){
    ...
    function events() {
        $("body")
            ...
            $.get(urlTmp)
                .done(function (datas) {
                    $('#StudentsModal').modal();
                    $("#StudentsModal .modal-body").html(datas);
                });
        })
            .on('click', '.change', function (e) {
            e.preventDefault();
            currentLineId = $(this).data('id');
            var urlTmp = url + '?action=get_edit_student_form&id=' + currentLineId;
            $.get(urlTmp)
                .done(function (datas) {
                    $('#ChangeModal').modal();
                    ...
                    $('#edit_student_birthdate').datetimepicker({
                        pickTime: false
                    });
                    $('#edit_student_deathdate').datetimepicker({
                        pickTime: false
                    });
                });
        })
            .on('click', '.delete_multiple', function () {
                var values = [];
                var checkedLines = $("#student-table input:checked[data-action!='check-all']");
            ...
            .on('click', '#deleteLine', function () {
                var urlTmp = url + '?action=delete_student&id=' + currentLineId;
                $.get(urlTmp)
                    .done(function (datas) {
                        datas = jQuery.parseJSON(datas);
                        if (datas.response) {
            ...
            .on('click', '#student-table .delete', function (e) {
                e.preventDefault();
                currentLineId = $(this).data('id');
                $('#DeleteModal').modal();
            })
            ...
            .on('click', '#change-submit', function(e){
                e.preventDefault();
                var urlTmp = url + '?action=change_student&id=' + currentLineId;
    ...
    $("#check-all").click(function(){
        if(unchecked)
            $("#student-table input[type='checkbox']").prop('checked',1);
        else
            $("#student-table input[type='checkbox']").prop('checked',false);
        unchecked = !unchecked;
    });
